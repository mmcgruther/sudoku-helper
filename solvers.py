...
class BasicSolver(object):
    ...
    def print_pencilmarks_board(self, board_candidates):
            ...
            for c_y in range(3):
                for x in range(9):
                    for c_x in range(3):
                        ...
                        val = (c_x+1)+3*(c_y)
                        val = str(val)
                        if val in board_candidates[x][y]:
                            print(val + " ", end = "")
                        else:
                            print(". ", end = "")
                    if x == 8:
    ...
    def solve_through_search(self, board_candidates):
        ...
        print("Checking space: " + str(x) + ", " + str(y) + " candidates: " + str(candidates))

        print(self.make_board_from_candidates(board_candidates))
        self.print_pencilmarks_board(board_candidates)
        input("Waiting...")
        new_i = self.make_board_from_candidates(board_candidates).get_symbol_count()
